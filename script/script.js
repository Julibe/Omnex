...
const ExifReaderLoader = {
    ...
    async ensureLoaded() {
        // Already loaded
        if (typeof window.ExifReader !== 'undefined' && window.ExifReader.load) {
            this.isLoaded = true;
            return true;
        }
    ...
    loadScript() {
        return new Promise((resolve, reject) => {
            ...
            script.src = 'https://unpkg.com/exifreader@4.12.0/dist/exif-reader.min.js';
            script.onload = () => {
...
class CustomVideoPlayer {
    ...
    bindEvents() {
        ...
        const progressTooltip = this.container.querySelector('.progress-tooltip');
        ...
        // Loading
        this.video.addEventListener('waiting', () => loading.classList.add('visible'));
        this.video.addEventListener('canplay', () => loading.classList.remove('visible'));
        ...
        progressContainer.addEventListener('mousemove', (e) => {
            const rect = progressContainer.getBoundingClientRect();
            const pos = (e.clientX - rect.left) / rect.width;
            progressTooltip.textContent = this.formatTime(pos * this.video.duration);
        ...
        const handleKeydown = (e) => {
            if (document.fullscreenElement !== wrapper && !wrapper.contains(document.activeElement)) return;
...
function renderAssetViewer(item) {
    ...
    const isVideo = ['mp4', 'webm', 'mov', 'mkv', 'm4v', 'avi', 'flv', 'wmv', 'mts', 'ts', 'ogv', 'vp9'].includes(ext);
    const isAudio = ['mp3', 'wav', 'ogg', 'flac', 'aac', 'm4a', 'opus', 'weba'].includes(ext);
    ...
    if (is3D) {
        ...
        setTimeout(() => new CustomVideoPlayer(document.getElementById('videoPlayer'), path, item.mime_type), 50);
    } else if (isAudio) {
...
async function loadExifData(url) {
    ...
    try {
        ...
        if (!loaded || typeof window.ExifReader === 'undefined') {
            throw new Error("Could not load ExifReader library. Check your internet connection.");
        }
        ...
        const ignoredTags = ['MakerNote', 'UserComment', 'Thumbnail', 'PrintIM', 'Padding'];
        ...
        for (let key in tags) {
            ...
            let category = 'Other';
            if (['ImageWidth', 'ImageHeight', 'Orientation', 'Resolution', 'Software', 'DateTime', 'Make', 'Model'].some(k => key.includes(k))) {
                category = 'Image Info';
            } else if (['Exposure', 'FNumber', 'ISO', 'FocalLength', 'Aperture', 'WhiteBalance', 'Flash'].some(k => key.includes(k))) {
